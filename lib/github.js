...
/**
 * Append a conversation exchange to contacts/logs/{slug}/YYYY-MM-DD.jsonl.
 * Creates intermediate directories implicitly (GitHub creates parent paths on PUT).
 */
export async function ghLogConversation(slug, userMessage, assistantMessage) {
  const date = new Date().toISOString().slice(0, 10);
  const path = `contacts/logs/${slug}/${date}.jsonl`;
  const record = {
    ts: new Date().toISOString(),
    user: userMessage,
    assistant: assistantMessage,
  };
  await ghAppendJsonl(path, record);
}

/**
 * Fetch a file and parse it as JSON. Returns null if not found.
 */
